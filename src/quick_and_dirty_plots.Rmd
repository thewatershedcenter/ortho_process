...
ctg <- readLAScatalog(lidar, select="xyzcrnw")
plots <- st_read(gpkg)
...
opt_output_files(ctg_aoi) <- file.path(tempdir(), "/{XLEFT}_{YTOP}")
...
ttops_path <- "/data/ttops2"
dir.create(ttops_path, showWarnings=FALSE)

crowns_path <- "/data/crowns2"
dir.create(crowns_path, showWarnings=FALSE)

dir.create(file.path(data, "disk_las2"), showWarnings=FALSE)
...
for (i  in 1:length(samples)) {
  ...
  chm <- rasterize_canopy(norm_las, 0.5, pitfree(max_edge=c(0, 1.5)))
